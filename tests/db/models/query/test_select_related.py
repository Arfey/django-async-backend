...
async def test_select_related_chained(async_db):
    ...
    reviews = [
        r async for r in Review.async_object.select_related("book__author")
    ]
...
async def test_select_related_with_filter(async_db):
    ...
    books = [
        b
        async for b in Book.async_object.select_related("author").filter(
            author__name="A1"
        )
    ]
    assert len(books) == 1
...
async def test_select_related_nullable_fk(async_db):
    ...
    notes = [
        n
        async for n in EditorNote.async_object.select_related("book").order_by("note")
    ]
    assert len(notes) == 2
...
async def test_select_related_self_referential(async_db):
    ...
    child = await TestModel.async_object.acreate(
        name="Child", value=2, relative=parent
    )

    children = [
        t
        async for t in TestModel.async_object.select_related("relative").filter(
            name="Child"
        )
    ]
...
async def test_select_related_with_order_by(async_db):
    ...
    books = [
        b
        async for b in Book.async_object.select_related("author").order_by(
            "author__name"
        )
    ]
    assert len(books) == 2
